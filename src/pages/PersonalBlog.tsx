...
import { 
  Tabs, Card, List, Tag, Input, Row, Col, Button, 
  Table, Modal, Form, Popconfirm, Select, Typography, 
  Avatar, Space, message, Divider 
} from 'antd';
import { 
  SearchOutlined, EyeOutlined, CalendarOutlined, 
  UserOutlined, ArrowLeftOutlined, EditOutlined, DeleteOutlined 
...
const PersonalBlog: React.FC = () => {
  ...
  return (
    <div style={{ padding: 24, background: '#fff', minHeight: '100vh' }}>
      ...
      <Tabs activeKey={activeTab} onChange={(key) => { setActiveTab(key); setViewingPost(null); }}>
        ...
        <Tabs.TabPane tab="Trang chủ" key="1">
          {viewingPost ? (
            ...
            <div style={{ maxWidth: 800, margin: '0 auto' }}>
              ...
              <img src={viewingPost.image} alt="cover" style={{ width: '100%', height: 300, objectFit: 'cover', borderRadius: 8 }} />
              <Title level={1} style={{ marginTop: 16 }}>{viewingPost.title}</Title>
              <Space split={<Divider type="vertical" />} style={{ marginBottom: 16, color: '#888' }}>
                <Text><UserOutlined /> {viewingPost.author}</Text>
                <Text><CalendarOutlined /> {viewingPost.date}</Text>
                <Text><EyeOutlined /> {viewingPost.views} lượt xem</Text>
              </Space>
              <div style={{ marginBottom: 24 }}>
                {viewingPost.tags.map((t: string) => <Tag color="blue" key={t}>{t}</Tag>)}
              </div>
              ...
              <Row gutter={16}>
                {getRelatedPosts(viewingPost).map(rp => (
                  <Col span={8} key={rp.id}>
                    <Card hoverable size="small" cover={<img alt="rel" src={rp.image} height={100} style={{objectFit: 'cover'}}/>} onClick={() => handleViewPost(rp)}>
            ...
            <div>
              ...
              <List
                ...
                pagination={{ pageSize: 9, position: 'bottom', align: 'center' }}
                      ...
                      <Card.Meta 
                        ...
                        description={
                          <>
                            ...
                            <div>{item.tags.map((t: string) => <Tag color="blue" key={t}>{t}</Tag>)}</div>
                          </>
                        } 
                      />
                    </Card>
        ...
        <Tabs.TabPane tab="Giới thiệu" key="2">
          <div style={{ textAlign: 'center', maxWidth: 600, margin: '0 auto', padding: '40px 0' }}>
            ...
            <Title level={2} style={{ marginTop: 16 }}>Đặng Hoàng Long</Title>
            <Text type="secondary">Web Developer / Blogger</Text>
            <Paragraph style={{ marginTop: 16, fontSize: 16 }}>
              Xin chào! Mình là sinh viên công nghệ thông tin đam mê lập trình Web. 
              Blog này là nơi mình chia sẻ những kiến thức, dự án và kinh nghiệm học tập hàng ngày.
            </Paragraph>
        ...
        <Tabs.TabPane tab="Quản lý Bài viết" key="3">
          <Button type="primary" style={{ marginBottom: 16 }} onClick={() => { setEditingPost(null); formPost.resetFields(); setIsPostModalVisible(true); }}>
            + Thêm bài viết mới
          </Button>
          <Table 
            dataSource={posts} 
            ...
            columns={[
              ...
              { title: 'Trạng thái', dataIndex: 'status', render: (s) => <Tag color={s === 'Published' ? 'green' : 'orange'}>{s}</Tag>, filters: [{ text: 'Nháp', value: 'Draft' }, { text: 'Đã đăng', value: 'Published' }], onFilter: (value, record) => record.status === value },
              ...
              { title: 'Thao tác', render: (_, record) => (
                <Space>
                  <Button size="small" icon={<EditOutlined />} onClick={() => { setEditingPost(record); formPost.setFieldsValue(record); setIsPostModalVisible(true); }} />
          ...
          />
          <Modal title={editingPost ? "Sửa bài viết" : "Thêm bài viết mới"} visible={isPostModalVisible} onCancel={() => setIsPostModalVisible(false)} onOk={() => formPost.submit()} width={800}>
            <Form form={formPost} layout="vertical" onFinish={handleSavePost}>
              <Row gutter={16}>
                ...
                <Col span={12}><Form.Item name="image" label="Ảnh đại diện (URL)"><Input /></Form.Item></Col>
                <Col span={6}><Form.Item name="status" label="Trạng thái" initialValue="Draft"><Select><Option value="Draft">Nháp</Option><Option value="Published">Đã đăng</Option></Select></Form.Item></Col>
                <Col span={6}><Form.Item name="tags" label="Thẻ Tag" initialValue={[]}><Select mode="multiple">{tags.map(t => <Option key={t.name} value={t.name}>{t.name}</Option>)}</Select></Form.Item></Col>
                <Col span={24}><Form.Item name="excerpt" label="Tóm tắt"><Input.TextArea rows={2} /></Form.Item></Col>
                <Col span={24}><Form.Item name="content" label="Nội dung (Markdown)" rules={[{ required: true }]}><Input.TextArea rows={6} /></Form.Item></Col>
              </Row>
            </Form>
          </Modal>
        </Tabs.TabPane>
        ...
        <Tabs.TabPane tab="Quản lý Thẻ (Tags)" key="4">
          <Button type="primary" style={{ marginBottom: 16 }} onClick={() => { setEditingTag(null); formTag.resetFields(); setIsTagModalVisible(true); }}>
            + Thêm thẻ mới
          </Button>
          <Table 
            dataSource={tags} 
            ...
            columns={[
              ...
              { title: 'Số bài viết sử dụng', render: (_, record) => <Tag color="blue">{getTagUsageCount(record.name)} bài viết</Tag> },
              { title: 'Thao tác', render: (_, record) => (
                <Space>
                  <Button size="small" icon={<EditOutlined />} onClick={() => { setEditingTag(record); formTag.setFieldsValue(record); setIsTagModalVisible(true); }} />
                  <Popconfirm title="Xóa thẻ này?" onConfirm={() => setTags(tags.filter(t => t.id !== record.id))}>
                    <Button size="small" danger icon={<DeleteOutlined />} disabled={getTagUsageCount(record.name) > 0} />
                  </Popconfirm>
                </Space>
              )}
            ]}
          />
          <Modal title={editingTag ? "Sửa thẻ" : "Thêm thẻ"} visible={isTagModalVisible} onCancel={() => setIsTagModalVisible(false)} onOk={() => formTag.submit()}>
